...
export const findpassword = function (req, res) {
  ...
  db.query(getUserInfo, username, (err, result) => {
    if (err) return res.send(err.message)
    if (!result.length) return res.status(401).send('用户不存在 !')
    if (result[0].email !== email) return res.status(404).send('邮箱错误 !')
    ...
    db.query(changePasswordStr, [hashPassword, result[0].id], (err) => {
